...
const checkAdminOrSupervisor = (context) => {
  ...
  if (
    role !== roles_1.USER_ROLES.ADMIN &&
    role !== roles_1.USER_ROLES.SUPERVISOR
  ) {
    throw new functions.https.HttpsError(
      "permission-denied",
...
exports.commitImport = functions.https.onCall(async (data, context) => {
  ...
  importedRows.forEach((row, idx) => {
    ...
    // 3. Resolve Branch
    const branchCol =
      mapping["branchName"] || mapping["branch_name"] || "BranchName";
  ...
  touchedRegionNos.forEach((regNo) => {
    const exists =
      existingAssignments.some((a) => a.regionNo === regNo) ||
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    chunk.forEach((rec) => {
      ...
      batch.set(
        db_1.db.collection("responses").doc(rec.recordId),
        newResponses[rec.recordId],
      );
    });
    if (i === 0) {
  ...
  touchedRegionNos.forEach((regNo) => {
    const addedCount = newRecords.filter(
      (r) => r.assignedRegionNo === regNo,
    ).length;
    if (addedCount > 0) {
  ...
  touchedRegionNos.forEach((regNo) => {
    ...
    if (rep) {
      notificationPromises.push(
        (0, notificationService_1.sendNotificationInternal)(
          rep.userId,
