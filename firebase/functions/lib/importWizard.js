...
exports.commitImport = functions.https.onCall(async (data, context) => {
    ...
    importedRows.forEach((row, idx) => {
        ...
        const regionVal = String(row[regKey] || row['regionNo'] || row['RegionNo'] || row['رقم المنطقة'] || row['رقم_المنطقة'] || row['المنطقة'] || '').trim();
        ...
        const customerNo = String(row[custNoKey] || row['customerNo'] || row['CustomerNo'] || row['رقم العميل'] || row['رقم_العميل'] || `CUST-${1000 + idx + 1}`).trim();
        ...
        const customerName = String(row[custNameKey] || row['customerName'] || row['CustomerName'] || row['اسم العميل'] || row['اسم_العميل'] || (lang === 'ar' ? `عميل ${idx + 1}` : `Customer ${idx + 1}`)).trim();
        // 3. Resolve Branch
        ...
        const defaultBranch = matchedUser
            ...
            : branches[0];
        const branchName = String(row[branchCol] || row['BranchName'] || row['Branch'] || row['الفرع'] || row['اسم الفرع'] || matchedUser?.branchNameAr || defaultBranch?.branchNameAr || '').trim();
        const branchId = matchedUser?.branchId || defaultBranch?.branchId || 'BR-01';
        // 4. Resolve Rep
        const repNameCol = mapping['repName'] || 'RepName';
        const repName = String(row[repNameCol] || row['RepName'] || row['اسم المندوب'] || row['المندوب'] || matchedUser?.repNameAr || '').trim();
        const repNoCol = mapping['repNo'] || 'RepNo';
        const repNo = String(row[repNoCol] || row['RepNo'] || row['رقم المندوب'] || matchedUser?.repNo || (regionVal ? `REP-${regionVal}` : '')).trim();
        ...
        reqFields.forEach((f) => {
            ...
            if (val === undefined && (f.fieldKey === 'customer_no' || f.fieldKey === 'cust_no'))
                val = customerNo;
            if (val === undefined && (f.fieldKey === 'customer_name' || f.fieldKey === 'cust_name'))
                val = customerName;
            if (val === undefined && (f.fieldKey === 'branch_name' || f.fieldKey === 'branch'))
                val = branchName;
