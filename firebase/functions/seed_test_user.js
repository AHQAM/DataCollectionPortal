...
async function seedTestUser() {
  ...
  if (!existing.empty) {
    console.log(
      `✅ User "${TEST_USER.username}" already exists (id: ${existing.docs[0].id})`,
    );
    process.exit(0);
  }
  ...
  console.log(
    `   Username: ${TEST_USER.username}  (this is the "Region No" field in the login screen)`,
  );
  console.log(
    `   Password: ${TEST_USER.password}  (will be forced to change on first login)`,
  );
