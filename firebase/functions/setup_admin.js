...
async function setupAdmin() {
  try {
    ...
    console.log(`Found user: ${adminAuthUser.email} (UID: ${adminAuthUser.uid})`);
    
    // 2. Set custom claim for ADMIN
    await admin.auth().setCustomUserClaims(adminAuthUser.uid, { role: 'ADMIN' });
    ...
    console.log(`Successfully created/updated user document in Firestore for ${adminAuthUser.uid}`);
