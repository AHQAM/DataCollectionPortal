...
export const AdminReports: React.FC = () => {
  const { lang, requests, records, branches, recordResponses, fields, t } =
    useApp();
  ...
  const [inspectingRecord, setInspectingRecord] = useState<RecordItem | null>(
    null,
  );

  // Selected choice field for dynamic chart / distribution
  const [selectedAnalysisFieldKey, setSelectedAnalysisFieldKey] =
    useState<string>("");
  ...
  const filteredRecords = records.filter((r) => {
    ...
    if (selectedBranchId !== "ALL" && r.branchId !== selectedBranchId)
      return false;
    if (selectedStatus !== "ALL" && r.recordStatus !== selectedStatus)
      return false;
    if (searchQuery.trim()) {
  ...
  const submittedCount = filteredRecords.filter(
    (r) => r.recordStatus === "Submitted",
  ).length;
  const draftCount = filteredRecords.filter(
    (r) => r.recordStatus === "DraftSaved",
  ).length;
  const pendingCount = filteredRecords.filter(
    (r) => r.recordStatus === "Pending",
  ).length;
  ...
  const activeAnalysisField =
    choiceFields.find((f) => f.fieldKey === selectedAnalysisFieldKey) ||
    choiceFields[0];
  ...
  if (activeAnalysisField) {
    filteredRecords.forEach((r) => {
      const resp = recordResponses[r.recordId] || r.rawData || {};
      const val =
        resp[activeAnalysisField.fieldKey] ??
        resp[activeAnalysisField.fieldId] ??
        "";
  ...
  const handleExportCSV = async () => {
    ...
    link.setAttribute(
      "download",
      `${currentRequest?.requestCode || "Report"}_Summary.csv`,
    );
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-lg font-extrabold text-slate-900 flex items-center gap-2">
            ...
            <span>{t("auto.reportsExcelExportCenter")}</span>
          </h1>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span>{t("auto.exportFullExcel")}</span>
          </button>
        </div>
      </div>
      ...
      {inspectingRecord && (
        <RecordResponseModal
          ...
          response={
            recordResponses[inspectingRecord.recordId] ||
            inspectingRecord.rawData
          }
