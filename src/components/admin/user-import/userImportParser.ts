...
export async function downloadCredentialsExcel(credentials: ImportedCredential[]): Promise<void> {
...
export function groupAndProcessRows(
  rawList: { repNo: string; repName: string; branchName: string; phone?: string }[],
  existingUsers: User[],
  lang: string
): ParsedRepRow[] {
...
export function parseExcelRows(
  jsonRows: any[]
): { rawList: { repNo: string; repName: string; branchName: string; phone?: string }[]; detectedSwap: boolean } {
