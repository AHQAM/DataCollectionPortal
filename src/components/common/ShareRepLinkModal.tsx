...
export const ShareRepLinkModal: React.FC<ShareRepLinkModalProps> = ({
  isOpen,
  onClose,
}) => {
      ...
      <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-purple-200 animate-in zoom-in-95 duration-150 space-y-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-black text-slate-900">
                {lang === "ar"
                  ? "مشاركة رابط تطبيق المندوبين"
                  : "Share Field Rep Application Link"}
              </h3>
        ...
        <div className="p-3.5 bg-emerald-50 border border-emerald-200 rounded-2xl flex items-start gap-2.5 text-xs">
          ...
          <div className="space-y-1 text-emerald-950">
            <div className="font-extrabold">
              {lang === "ar"
                ? "حماية وعزل الصلاحيات التام (Role Isolation):"
        ...
        <div className="space-y-2">
          <label className="block text-xs font-bold text-slate-700">
            {lang === "ar"
              ? "رابط تطبيق المندوب الميداني (انسخ وأرسل):"
              : "Field Rep Link (Copy & Send):"}
          </label>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-1">
          ...
          >
            ...
            <span>
              {lang === "ar" ? "إرسال عبر الواتساب" : "Share via WhatsApp"}
            </span>
          </a>
        ...
        <div className="bg-slate-50 rounded-2xl p-3 border border-slate-200 text-xs space-y-2">
          <div className="font-bold text-slate-700 flex items-center justify-between">
            <span>
              {lang === "ar"
                ? "أرقام مناطق المناديب المسجلة بالنظام:"
                : "Registered Rep Region Accounts:"}
            </span>
            <span className="text-[10px] text-purple-700 font-bold">
              {repUsers.length} مناديب
            </span>
          </div>
          <div className="flex flex-wrap gap-1.5">
            {repUsers.slice(0, 6).map((u) => (
              ...
              >
                {lang === "ar" ? `منطقة ${u.regionNo}` : `Region ${u.regionNo}`}{" "}
                ({u.repNameAr.split(" ")[0]})
              </span>
            ))}
          </div>
