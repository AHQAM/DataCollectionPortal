  ...
  return {
    ...
    approveDeviceReplacement: async (userId: string, reason?: string) => {
      try {
        ...
        logAudit("DEVICE_REPLACEMENT_APPROVED", "DeviceBinding", userId, {
          reason,
        });
        return { success: true };
      } catch (err: any) {
    ...
    // ==================== System Maintenance ====================
    wipeDemoDataForProduction: async (options?: {
      wipeBranchesAndRegions?: boolean;
    }) => {
      try {
        const res = await systemApi.wipeDemoData(
          "CONFIRM_WIPE_DEMO_DATA",
          options?.wipeBranchesAndRegions,
        );
