...
interface AuthStore {
  ...
  changePassword: (currentPassword: string, newPassword: string, minLength: number) => Promise<{ success: boolean; message: string }>;
  adminResetPassword: (userId: string) => Promise<{ success: boolean; temporaryPassword?: string; message: string }>;
...
export const useAuthStore = create<AuthStore>((set, get) => {
  ...
  return {
    ...
    login: async (regionNoOrEmail: string, passwordInput: string): Promise<LoginResult> => {
      ...
      try {
        if (trimmedInput.includes('@')) {
          ...
          if (data.success && data.token) {
              ...
              return { success: true, mustChangePassword: loggedInUser.mustChangePassword };
          } else {
              return { success: false, messageAr: 'بيانات تسجيل الدخول غير صحيحة.', messageEn: 'Invalid login credentials.' };
          }
        }
      } catch (err: any) {
    ...
    changePassword: async (currentPassword: string, newPassword: string, minLength: number) => {
    ...
    adminResetPassword: async (userId: string) => {
      ...
      try {
        ...
        return { success: true, temporaryPassword, message: 'Temporary password created.' };
      } catch (error) {
